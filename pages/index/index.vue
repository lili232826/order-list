<template>
	<view>
       <div class="mui-content hg-content">
			...
			<view class="hg-list mui-col-xs-9">
				<div class="" id="segmentedControlContents" style="height: 100%;" ref="rightWrapper">
					<scroll-view scroll-y style="height: 100%;position: static !important;" :scroll-with-animation='true'  :scroll-into-view="toView" @scrolltoupper="upper" @scroll="scroll">
								...
								<li v-for="(list,ind) in item.menu" :data-id="list.id" class="mui-table-view-cell">
									<a href="javascript:;">
										...
										<div class="mui-media-body">
											{{list.name}}{{index}}
											<div class="uni-flex">
												<p class=''>￥<span>{{list.price}}</span></p>
												<div class=' checkbox'>
													<span class="span">
														<button class="button check-reduce" v-if="list.chartNum>0" @tap="listjian(list.id)"></button>
													</span>
													<span class="num" v-if="list.chartNum>0">{{list.chartNum}}</span>
													<span class="span">
														<text class="button check-add" @tap="listAdd" v-bind:id="list.id"></text>
													</span>
												</div>
											</div>
										</div>
	   ...
	   <footer class="cart-foot" id="cart-foot">
	   	<div class="cart-wrap">
	   		<div id="cart" class="cart" :class="allNum>0 ? 'active' : ' ' " @tap="showList()">
							...
							<li class="mui-table-view-cell" v-for="list in checkedList">
								<div class="list">
									...
									<div class='mui-ellipsis checkbox'>
										<span class="span">
											<button class="button check-reduce" @tap="listjian(list.id)"></button>
										</span>
										<span class="span">{{list.chartNum}}</span>
										<span>
											<text class="button check-add"  @tap="listAdd" v-bind:id="list.id"></text>
										</span>
									</div>
...
var rightInterval=25;//切换之间的间距，小标题的高度
	export default {
		...
		onReady(){
			var cart=document.getElementById('cart-foot');
			this.cartPos[0]=cart.offsetLeft*1+45;
			this.cartPos[1]=cart.offsetTop*1-10;
		},
		methods: {
			...
			scroll:function(e){
				this.sLong=e.target.scrollTop//滚动的距离；
			...
			listAdd:function(e){
				var id=e.target.id,target=e.target;
				let Vue=this;
				for (let i=0;i<this.menuList.length;i++){
					let item=this.menuList[i];
					for(let j=0;j<item["menu"].length;j++){
			...
			//球的动作
			addboll:function(target){
				var target=target;
				//var timer = setTimeout(function () {
					let left=target.offsetLeft,top=target.offsetTop-(this.sLong)+40;
					console.log(left,top);
					//创建球
					var out=document.createElement("div"),inner=document.createElement("div");
					out.className='outer';inner.className='inner';
					...
					// 计算坐标差
					var leftDifference = -(left - this.cartPos[0]);
					var topDifference = -(top - this.cartPos[1]);
					console.log(this.cartPos[0],this.cartPos[1],leftDifference,topDifference,"kkkkkkkkkkkk")
					setTimeout(function () {
					//console.log($(".inner")[0],"inner")
							inner.style.transform = `translateY(${topDifference}px)`;
							inner.style.width = "10px";
							inner.style.height = "10px";
							out.style.transform = `translateX(${leftDifference}px)`;
							setTimeout(function () {
